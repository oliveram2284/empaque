...
function BuscarViajes($dia, $mes, $anio, $clase){
    ...
    if(mysql_num_rows($resu) > 0){    	
    	...
    	while($row = mysql_fetch_array($resu))
           {
           	$via = "Select Count(*) As Cant From prioridad Where viajeId = ".$row['idViaje']." and usrId = ".$_SESSION['id_usuario'];
	    	...
	    	if(mysql_num_rows($resu_) > 0){    	
	    		while($row_ = mysql_fetch_array($resu_))
           			...
           			if($hay_listados == false)
						$html .= '<div style="width: 110px; font-size: 15px; color: white; background:'.$row['color'].'; box-shadow: 2px 2px 5px #999; margin-bottom: 5px; padding: 5px; cursor: pointer;" onclick="AbrirPorViaje('.$row['idViaje'].')" >'.$row['descripcion'].'</div>';
					else
						$html .= '<div style="width: 110px; font-size: 15px; color: white; background:'.$row['color'].'; box-shadow: 2px 2px 5px #999; margin-bottom: 5px; padding: 5px; cursor: pointer;" onclick="AbrirPorViaje('.$row['idViaje'].')">
									'.$row['descripcion'].'<img src="./assest/plugins/buttons/icons/abajo.png" width="20" heigth="20" >
								 </div>';
			$hay_listados = false;
           }
         $html .= "</td>";
...
function armarSemaforo($idViaje)
{
	//Contar todos
	$todos = 0;
	$sql = 'Select Count(DISTINCT d.pedId) As Cant From prioridad As p Join prioridaddetalle As d On p.prioId = d.prioId Where p.viajeId = '.$idViaje.' and cantAut is not null and cantAut > -1 and p.usrId = '.$_SESSION['id_usuario'];
	...
	$aprobados = 0;
	$sql = 'Select Count(DISTINCT d.pedId) As Cant From prioridad As p Join prioridaddetalle As d On p.prioId = d.prioId Where p.viajeId = '.$idViaje.' and cantAut is not null and d.cantEnt is not null and cantAut > -1 and p.usrId = '.$_SESSION['id_usuario'];
	$resu_ = mysql_query($sql);
	if(mysql_num_rows($resu_) > 0){    	
		while($row_ = mysql_fetch_array($resu_))
   			{
