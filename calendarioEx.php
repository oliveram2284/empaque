...
if($year >= date("Y")){
	...
	else
	{
		...
		{
			$html .= '<button class="btn btn-mini btn btn-info" type="button" onclick="AbrirCalendario()" style="margin-top: 5px" disabled="disabled">
						 <i class="icon-chevron-left icon-white"></i>
					  </button>
					  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					  <button class="btn btn-mini btn btn-info" type="button" onclick="AbrirCalendario('.($month + 1).','.$year.')" style="margin-top: 5px">
...
function armarSemaforo($idViaje)
{
	//Contar todos
	$todos = 0;
	$sql = 'Select Count(DISTINCT d.pedId) As Cant From prioridad As p Join prioridaddetalle As d On p.prioId = d.prioId Where p.viajeId = '.$idViaje.' and cantAut is not null and cantAut > -1';
	...
	$aprobados = 0;
	$sql = 'Select Count(DISTINCT d.pedId) As Cant From prioridad As p Join prioridaddetalle As d On p.prioId = d.prioId Where p.viajeId = '.$idViaje.' and cantAut is not null and d.cantEnt is not null and cantAut > -1';
	$resu_ = mysql_query($sql);
	if(mysql_num_rows($resu_) > 0){    	
		while($row_ = mysql_fetch_array($resu_))
   			{
