 ...
 <div class="container">
	...
	<hr>
		<div class="row">
			...
			Día: <input type="text" style="width: 100px" readonly id="ViajeDia"> 
			Destino: <input type="text" readonly id="ViajeDestino"> 
			Empresa: <input type="text" readonly id="ViajeEmpresa">
			<button class="btn btn-mini btn btn-info" type="button" onclick="AbrirCalendario( <?php echo date("n") .",". date("Y");?> )" style="margin-top: -9px">
				<i class="icon-search icon-white"></i>
			</button>

			<button class="btn btn-mini btn btn-success" type="button" onclick="CrearPrioridad()" style="margin-top: -9px">
				<i class="icon-folder-close icon-white" style="margin-top: -2px"></i> Guardar
			</button>
		</div>
	<hr>
...
<!--------- End Pop Info -->
 ...
 <!-- Pop Calendarío -->
<div class="modal hide fade" id="CalendarioPop" style="width: 900px; margin-left: -450px; margin-top: -400px;">
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
    <h3>Buscador de Viajes</h3>
  </div>
  ...
  <div class="modal-footer">
    <a href="#" class="btn" onclick="ClosePop('CalendarioPop')">Cerrar</a>
  </div>
</div>
	...
	if(
           ...
           !(document.getElementById('Usuario').checked)
          )	
			{
				...
				$("#campo_ord").val(value);
				reporteAjax('div_find','prioridadEntregaphp.php',filtros);
			}
			else
				{
					var filtros = "";
...
function SeleccionarViaje(idViaje, Fecha, Empresa, Destino){

	$("#ViajeId").val(idViaje);
	$("#ViajeDia").val(Fecha); 
	$("#ViajeDestino").val(Destino); 
	$("#ViajeEmpresa").val(Empresa);
		
	$("#CalendarioPop").modal('hide');
}

var pedidosId = new Array();

function AgregarOQuitar(id, object){
	...
	//Remover de la lista
	pedidosId.splice(pedidosId.indexOf(id), 1);
    }
}
